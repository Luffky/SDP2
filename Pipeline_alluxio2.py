...
def extract_lsm_kernel(ix):
	...
	Hash = hash(label)
	input_size = 0
	print(label + " (hash " + Hash + " from " + str(input_size / 1000000) + " MB input)")
...
def telescope_data_kernel(x):
	...
	Hash = int(hash(label), 16)
	input_size = 0
	for data in data_telescope_management:
		Hash ^= int(hash(bytes(data[0:4])), 16)
		input_size += len(data)
	print(label + " (hash " + str(Hash) + " from " + str(input_size / 1000000) + " MB input)")
	result = bytearray(max(4, int(scale_data * 0)))
	#ByteBuffer.wrap(result).putInt(0, hash) 
	return (ix, result)
